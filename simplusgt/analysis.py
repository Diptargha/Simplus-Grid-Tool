...
def bus_strength(list_line: np.ndarray, apparatus_types: list[int]) -> tuple[np.ndarray, np.ndarray]:
    n_bus = int(max(np.max(list_line[:, 0]), np.max(list_line[:, 1]))) if list_line.size else len(apparatus_types)
    ...
    for row in list_line:
        ...
        r, x, b, g, tap, area = row[2], row[3], row[4], row[5], row[6], int(row[7])
        if area == 1:
            y = g + 1j * b + 1 / (r + 1j * x)
        else:
            y = g + 1 / r
        if f != t:
            ydiag[f] += y / tap ** 2
            ydiag[t] += y
            graph_matrix[f, t] = graph_matrix[t, f] = abs(y)
        else:
            ydiag[f] += y
